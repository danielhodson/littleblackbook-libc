// Compile as:
// clang dns.c -o dns -O1 -g -fsanitize=memory,fuzzer,undefined -fno-omit-frame-pointer
// Run as:
// > ./dns -max_len=512 -use_value_profile=1 -shrink=1 -jobs=8 -print_pcs=1 INPUT_CORPUS/ OUTPUT_CORPUS/
// Or help with:
// ./dns -help=1
...
int dns_parse(const unsigned char *r, int rlen, int (*callback)(void *, int, const void *, int, const void *), void *ctx)
{
	int qdcount, ancount;
...
static int dns_parse_callback(void *c, int rr, const void *data, int len, const void *packet)
{
        ...
        if (dn_expand(packet, (const unsigned char *)packet + 512,
            data, c, 256) <= 0)
                *(char *)c = 0;
        return 0;
...
int getnameinfo(const uint8_t *Data, size_t Size) {
  ...
  return dns_parse(reply, rlen, dns_parse_callback, buf);
}

int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
        int ret = EXIT_SUCCESS;
        ret = getnameinfo(Data, Size);
        return ret;
}
